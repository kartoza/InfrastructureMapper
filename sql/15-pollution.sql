...
CREATE TABLE IF NOT EXISTS pollution_impact (
    ...
    pollution_source_id  NOT NULL REFERENCES pollution_source (id),
    waterbody_id NOT NULL REFERENCES waterbody_type (id),
    pollutant_id NOT NULL REFERENCES pollutant (id),
    impact_severity_id NOT NULL REFERENCES impact_severity (id)  
);
...
CREATE TABLE IF NOT EXISTS pollution_source (
    ...
    last_update_by TEXT NOT NULL,
    uuid UUID UNIQUE NOT NULL DEFAULT gen_random_uuid(),
    property_use_type_id NOT NULL REFERENCES property_use_type (id)
);
...
COMMENT ON COLUMN source_pollutant.pollution_source IS 'The foreign key which references the id from the pollution_source table.';
...
CREATE TABLE IF NOT EXISTS waterbody (
    ...
    last_update_by TEXT NOT NULL,
    uuid UUID UNIQUE NOT NULL DEFAULT gen_random_uuid(),
    waterbody_type_id NOT NULL REFERENCES waterbody_type (id),
    water_quality_status_id NOT NULL REFERENCES water_quality_status (id)
);
...
COMMENT ON TABLE water_quality_staus IS 'Lookup table for water quality for waterbodies in the area of interest, e.g. "Good", "Poor".';
...
COMMENT ON COLUMN water_quality_status.notes IS 'Additional information of water quality'.;
