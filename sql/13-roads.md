...
| `last_update` | `timestamp without time zone` | no | `now()` | The date that the last update was made (yyyy-mm-dd hh:mm:ss). |
| `last_update_by` | `text` | no |  | The name of the user responsible for the latest update. |
| `geom` | `USER-DEFINED` | no |  | The location of the nodes between road segments. EPSG: 4326 (WGS 84). Use --crs at build time to reproject for accurate distance measurements. |
...
| `geom` | `USER-DEFINED` | no |  | Centreline location of the road segment. EPSG: 4326 (WGS 84). Use --crs at build time to reproject into a metric CRS for accurate length measurements. |
